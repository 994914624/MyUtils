 ...
 *
 * 单例模式，需要get出实例。具体缩放比例可以自己去定。
 */
public class MyThumbnailCache_InternalUtils {
    //内存存储处理对象
    ...
    private int type=CACHE;
    //用于修改存储位置，0代表cache目录，1代表file目录
    ...
    private static MyThumbnailCache_InternalUtils myThumbnailCacheInternalUtils;
     ...
     *
     * */
    public static MyThumbnailCache_InternalUtils getImageCacheHelper(Context context) {
        MyThumbnailCache_InternalUtils.context=context;
        if (MyThumbnailCache_InternalUtils.myThumbnailCacheInternalUtils == null) {
            myThumbnailCacheInternalUtils = new MyThumbnailCache_InternalUtils();
        }
        return MyThumbnailCache_InternalUtils.myThumbnailCacheInternalUtils;
    }

    private MyThumbnailCache_InternalUtils() {
        //用于获取内部存储目录的上下文对象
     ...
     *  此方法，把url，imageView传进去，然后获得略缩图放在imageView。
     *
     *
     * */
    ...
    public void loadImage(String url, ImageView imageView) {
        ...
        if (bitmap != null) {
            ...
            if (file.exists()) {
                ...
                bitmap =createThumbnail(file.getAbsolutePath(),imageView.getMeasuredWidth(),imageView.getMeasuredHeight());
            }
            //判断bitmap是否在sd卡上加载成功
            if (bitmap != null) {
                //在sd卡上加载了bitmap
    ...
    class ImageAsyncTask extends AsyncTask<String, Void, Bitmap> {
        ...
        protected Bitmap doInBackground(String... params) {
            ...
            if (data != null) {
                ...
                switch (type){
                    case CACHE:
                        otherWriter(fileName,data,context,CACHE);
                        break;
                    case FILES:
                        otherWriter(fileName,data,context,FILES);
                        break;
                }
