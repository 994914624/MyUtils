 ...
 *
 * 三级缓存+二次采样，网络获取的图片储存在SD卡。单例模式，需要get出实例。具体缩放比例可以自己去定。
 *
 * 这个储存到SD卡要注意，SD卡是否好用，目录是否存在！若不存在，创建这个文件夹；
 *
 *
 */
public class MyThumbnailCache_SDCardUtils {
    //内存存储处理对象
    private MyLruCache myLruCache = null;


    //使用的时候，自己去注意线程同步安全问题，也就是在application对象中去实例这个
    private static MyThumbnailCache_SDCardUtils myThumbnailCacheSDCardUtils;
     ...
     *
     * */
    public static MyThumbnailCache_SDCardUtils getMyThumbnailCacheSDCardUtils() {
        if (MyThumbnailCache_SDCardUtils.myThumbnailCacheSDCardUtils == null) {
            myThumbnailCacheSDCardUtils = new MyThumbnailCache_SDCardUtils();
        }
        return MyThumbnailCache_SDCardUtils.myThumbnailCacheSDCardUtils;
    }

    private MyThumbnailCache_SDCardUtils() {
        //获取到这个应用程序的总内存大小
