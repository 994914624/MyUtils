...
public class MySQLiteOpenHelperUtils extends SQLiteOpenHelper {

	private static final String DB_NAME = "db_openhelper.db";
	...
	/**
	 * 数据库默认位置：内部存储data/data/包名/databases/数据库名字
	 * 
	 * 构造方法，要求一定要调用父类的构造方法，而父类的构造方法有四个参数，
	        这里为了方便使用，我们对于父类的构造方法进行封装(改变)
	 * 
	 * 
	 * */
	public MySQLiteOpenHelperUtils(Context context) {
		// 第一个参数是上下文
		// 第二个参数是数据库的名字
	 ...
	 *  版本更新
	 *  把原来的直接删除
	 *  （更新数据库的时候，有可能要数据备份，ListMap拿到，然后再插入到新表中。保不保存之前的数据，结合需求。）
	 * */
	@Override
